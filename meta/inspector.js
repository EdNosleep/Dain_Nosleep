// ===============================
// Dain_Coin — INSPECTOR (v26)
// Shared TrayPanelLayout
// ===============================
//
// CHANGELOG v26:
// • Inspector переведён на общий trayPanelLayout.
// • Заголовок Inspector теперь фиксированный.
// • Reset перенесён в layout.actions.
// • Scroll и transparent mask теперь работают только для body под заголовком.
// • Runtime persistence / hard reset / enabled modules сохранены.
// • Сохранён scroll fix из v25.
//
// ===============================

import { defineTrayButton } from "../ui/tray/defineTrayButton.js";
import { createTrayPanelLayout } from "../ui/tray/trayPanelLayout.js";

const STORAGE_KEY = "DainCoin_Inspector_v25";
...
function mountInspector({ container, core, storageKey }) {
  ...
  if (typeof core?.setInspectorStorageKey === "function") {
    core.setInspectorStorageKey(storageKey);
  }

 // syncStoredParamsToCore(core, stored);
  ...
  const getEnabledStored = moduleKey => {
    const k = `enabled::${moduleKey}`;
    return Object.prototype.hasOwnProperty.call(stored, k)
      ? !!stored[k]
      : null;
  };

  const setEnabledStored = (moduleKey, value) => {
    stored[`enabled::${moduleKey}`] = !!value;
    saveStored(storageKey, stored);
  };

  const clearStored = () => {
    for (const key in stored) delete stored[key];
  ...
  resetBtn.onclick = e => {
    ...
    if (typeof core.resetModulesToDefaults === "function") {
      core.resetModulesToDefaults();
    }

    rebuild();
  };

  layout.actions.appendChild(resetBtn);
    ...
    core.getModules().forEach(({ key, module }) => {
      ...
      list.appendChild(
        createCollapsibleModule({
          ...
          getStoredEnabled: () => getEnabledStored(key),
          setStoredEnabled: value => setEnabledStored(key, value),
  ...
  return () => {
    ...
    if (previousContainerStyle === null) {
      container.removeAttribute("style");
    } else {
      container.setAttribute("style", previousContainerStyle);
    }
  };
}
...
function syncStoredParamsToCore(core, stored) {
  if (!core || !stored) return;
  ...
  core.getModules().forEach(({ key, module }) => {
    ...
    for (const label in module.inspector) {
      ...
      if (Object.prototype.hasOwnProperty.call(stored, storageKey)) {
        core.applyParam(key, def.param, stored[storageKey]);
      }
    }
  });
}
...
function createCollapsibleModule({
  core,
  bus,
  moduleKey,
  title,
...
function createCollapsibleControl({
  bus,
  moduleKey,
  param,
  label,
...
function createInput(def, value, onChange, { bus = null, meta = null } = {}) {
  ...
  if (bus && typeof bus.emit === "function") {
    ...
    if (handled.value) return placeholder;
  }
