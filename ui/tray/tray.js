// ======================================
// Dain_Coin — TRAY MODULE (v10)
// Clean button open flow for persistent TrayPanel
// ======================================
//
// CHANGELOG v10:
// • Убран принудительный tray:setPanelLevel({ level: 1 }) при клике по кнопке
// • Убран пустой tray:openPanel перед onClick кнопки
// • Кнопка теперь только активируется и отдаёт управление своему onClick
// • Это позволяет trayPanel сохранять текущий уровень при смене вкладок
// • Сохранены жесты, glow, brightness и текущий визуал v9
//
// ======================================
...
function applyTrayPosition(state) {
  ...
  const baseBottom = (state.panelOffsetPx || 0) + state.params.marginBottom;
  state.tray.style.bottom = `calc(var(--safe-bottom, 0px) + ${baseBottom}px)`;
}

function applyTrayMotion(state) {
  if (!state.tray) return;
  state.tray.style.transition = `bottom ${state.animDuration}ms ease`;
}
