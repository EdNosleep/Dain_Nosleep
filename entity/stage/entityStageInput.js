// ======================================================
// NOSLEEP_ENGINE — entityStageInput.js v2
// Focus-safe private input helper for Entity Stage 3D
// ======================================================
//
// CHANGELOG v1:
...
//
// CHANGELOG v2:
// • Во Focus StageInput больше не снимает selection на pointerdown по пустоте.
// • Во Focus StageInput больше не выбирает другие объекты.
// • Свайп по свободной зоне во Focus продолжает вращать камеру вокруг cameraTarget.
// • Выход из Focus остаётся ответственностью entityStageFocusInput.
// • Обычный selection вне Focus сохранён.
// ======================================================
...
function onPointerDown(e, state) {
  ...
  const inFocus = isFocusActive(state);
  ...
  if (inFocus) {
    handleFocusPointerDown(state, hit);
    return;
  }
...
function handleFocusPointerDown(state, hit) {
  const selectedNodeId = state.selectedNodeId;
  const selectedMesh = selectedNodeId ? state.meshes.get(selectedNodeId) : null;

  if (hit && selectedMesh && belongsToMesh(hit, selectedMesh) && state.transformMode === "move") {
    beginObjectMoveDrag(state, hit, state.lastX, state.lastY);
    state.draggingObject = true;
    state.draggingCamera = false;
    return;
  }
...
function isFocusActive(state) {
  const phase = state.focusInput?.phase || "idle";
  return phase === "entering" || phase === "focused";
}
