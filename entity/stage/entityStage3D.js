// ======================================================
// NOSLEEP_ENGINE — entityStage3D.js v28
// Stage Coordinator + Focus Mode helpers
// ======================================================
...
import { refreshMeshColors } from "./entityStageMaterials.js";
...
import {
  ...
  emitStageReady,
  emitStageRuntime,
  emitStageRuntimeDisposed
} from "./entityStageRuntimeApi.js";

import {
  bindStageProjectionApi,
  emitSelectedProjection
} from "./entityStageProjectionApi.js";
...
} from "./entityStagePreview.js";

import {
  renderDraft,
  rebuildAllMeshGeometry,
  disposeAllDraftMeshes
} from "./entityStageDraft.js";

import {
  bindStageFocusCamera,
  destroyStageFocusCamera
} from "./entityStageFocusCamera.js";

import {
  bindStageFocusInput,
  destroyStageFocusInput
} from "./entityStageFocusInput.js";

import {
  bindStageFocusRenderLayer,
  destroyStageFocusRenderLayer
} from "./entityStageFocusRenderLayer.js";
...
export const registerEntityStage3DModule = defineModule({
  ...
  inspector: {
    ...
    "Base Grid прозрачность": { param: "baseGridOpacity", type: "range", min: 0, max: 1, step: 0.05, value: 0.35 },
    ...
    "Яркость змей": { param: "environmentLineOpacity", type: "range", min: 0, max: 100, step: 1, value: 42 },
    "Glow цвет": { param: "environmentGlowColor", type: "color", value: "#000000" },
    "Glow яркость": { param: "environmentGlowBrightness", type: "range", min: 0, max: 100, step: 1, value: 32 },
    "Количество змей": { param: "environmentSnakeCount", type: "range", min: 1, max: 140, step: 1, value: 90 },
    "Мин. скорость змей": { param: "environmentSnakeMinSpeed", type: "range", min: 1, max: 160, step: 1, value: 34 },
    "Макс. скорость змей": { param: "environmentSnakeMaxSpeed", type: "range", min: 1, max: 240, step: 1, value: 96 },
    "Множитель скорости": { param: "environmentSnakeSpeedMul", type: "range", min: 50, max: 400, step: 5, value: 220 },
    "Длина сегмента": { param: "environmentSnakeSegmentLength", type: "range", min: 40, max: 700, step: 10, value: 390 },
    "Длина хвоста": { param: "environmentSnakeTrailLength", type: "range", min: 8, max: 140, step: 1, value: 80 },
    "Толщина змей": { param: "environmentSnakeThickness", type: "range", min: 0.3, max: 8, step: 0.1, value: 2 },
    "Сужение хвоста": { param: "environmentSnakeTaper", type: "range", min: 0.4, max: 4, step: 0.05, value: 1.45 },
    "Мин. время поворота": { param: "environmentSnakeTurnMin", type: "range", min: 0.05, max: 4, step: 0.05, value: 0.35 },
    "Макс. время поворота": { param: "environmentSnakeTurnMax", type: "range", min: 0.1, max: 8, step: 0.05, value: 1.65 },
    "Шанс квадрата": { param: "environmentSnakeLoopChance", type: "range", min: 0, max: 50, step: 1, value: 3 },
    "Глубина параллакса": { param: "environmentParallaxDepth", type: "range", min: 0, max: 700, step: 5, value: 220 },
    "Плавность параллакса": { param: "environmentParallaxSmooth", type: "range", min: 1, max: 100, step: 1, value: 24 },
    "Глубина слоя": { param: "environmentDepthStrength", type: "range", min: 0, max: 900, step: 5, value: 220 },
    "Размер поверхности": { param: "environmentSurfaceScale", type: "range", min: 120, max: 420, step: 10, value: 260 },
    "Прозрачность Environment": { param: "environmentGlobalOpacity", type: "range", min: 0, max: 100, step: 1, value: 100 },
    "Pixel Ratio Limit": { param: "environmentPixelRatio", type: "range", min: 1, max: 2, step: 0.05, value: 1.35 },
    ...
    "Preview прозрачность": { param: "previewOpacity", type: "range", min: 0.15, max: 1, step: 0.05, value: 0.62, hidden: true },
    "Preview цвет": { param: "previewColor", type: "color", value: "#ffd36a", hidden: true },
  ...
  onStart({ ctx, state, bus, store }) {
    ...
    bindStageRuntimeApi(state, bus);
    bindStageProjectionApi(state, bus);
    bindStagePreviewApi(state, bus);

    bindStageFocusCamera(state, bus);
    bindStageFocusInput(state, bus);
    bindStageFocusRenderLayer(state, bus);

    emitStageReady(state);
    emitStageRuntime(state, null);
  ...
  onDisable({ state }) {
    emitStageRuntimeDisposed(state);

    state.running = false;
    cancelAnimationFrame(state.raf);
    state.raf = 0;

    destroyStageFocusRenderLayer(state);
    destroyStageFocusInput(state);
    destroyStageFocusCamera(state);
  ...
  onParam({ param, value, state }) {
    ...
    if (param === "viewportLiftFactor" || param === "viewportOverscanBottom") {
...
// CHANGELOG v28:
// • Подключены Focus helpers: Camera / Input / RenderLayer.
// • Повторный тап по выбранному объекту теперь может входить во Focus Mode.
// • Во Focus тап вне Gizmo может запрашивать выход из Focus.
// • Во Focus выбранный объект и Gizmo получают визуальный приоритет.
// • entityStage3D.js остаётся координатором lifecycle/wiring.
// • Runtime API, Projection API, Preview и Draft helpers сохранены.
// • Функционал v27 сохранён.